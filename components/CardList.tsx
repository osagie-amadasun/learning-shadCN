  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col gap-2">
        {list.map((item) => (
          <Card key={item.id} className="flex-row items-center justify-between gap-1 p-2">
            ...
            <CardContent className="p-0 ml-3 flex-1">
              <CardTitle className="text-sm font-medium">{item.title}</CardTitle>
              <Badge variant="secondary">{item.badge}</Badge>
            </CardContent>
            <CardFooter className="p-0 text-sm font-medium">{item.count / 1000}K</CardFooter>
          </Card>
        ))}
      </div>
    </div>
